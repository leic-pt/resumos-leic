...
Para ser possível fazer um envio de um pacote de um host A para um host B, existem 2 desafios distintos:
...
- O núcleo B tem 5 pessoas, então a IST fornece os endereços `12.0.0.0/29`, ou seja, os endereços `12.0.0.0` até `12.0.0.7` (em bits, `00001100.00000000.00000000.00000 000` até `00001100.00000000.00000000.00000 111`). Foram dados 8 IPs mas, como 3 não podem ser atribuídos a _hosts_, o núcleo só tem efetivamente 5 IPs.
  :::
...
- configurar estaticamente o NAT - Desta forma, as solicitações de conexão de entrada são encaminhadas numa determinada porta para o servidor. Por exemplo, o pacote (`123.76.29.7`, `2500`) é sempre encaminhada para a porta `25000` do IP `10.0.0.1`;
...
endereços IPv4 tinham apenas 32).
...
apenas têm que se preocupar com fazer o routing.
...
Este algoritmo usa um algoritmo de _Link State_ e permite ter uma relação hierárquica, ou seja, permite ter ASs dentro de ASs.

...
- [**Mapeamento**](color:orange): Os valores existentes numa tabela são aumentados
  com um mapeamento para valores de outra tablea (e.g. na tabela de compras,
...
- [**Filtragem**](color:green): Certas linhas de uma tabela não são selecionadas
  ...
  de alunos com uma tabela de notas, pelo que os alunos sem notas são aparecem).
...
-- Right Outer Join: Incluir todas as linhas da tabela 'weather'
SELECT weather.city_name, weather.temperature,
       city.city_name, city.location
  FROM weather
...
-- Full Outer Join: Incluir todas as linhas da tabela 'weather'
SELECT weather.city_name, weather.temperature,
       city.city_name, city.location
  FROM weather
...
O `NATURAL JOIN` permite efetual _joins_ de uma forma mais simples, sendo apenas
